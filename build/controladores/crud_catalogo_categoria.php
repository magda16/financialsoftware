  ...
  if(isset($_POST["bandera"])){
    ...
    if($bandera=="add"){
      ...
      function obtenerResultado(){
        ...
        $stmt = $pdo->prepare("SELECT MAX(id_activo_categoria)+1 AS 'id' FROM activo_categoria");
        $stmt->execute();
        $result=$stmt->fetchAll(PDO::FETCH_ASSOC);  
        if ($result) {
          foreach($result as $get_id){
            $id_activo_categoria=$get_id['id'];
          }
        }

        if($id_activo_categoria==null){
          $id_activo_categoria=1;
        }
        ...
        $stmt=$pdo->prepare("INSERT INTO activo_categoria (id_activo_categoria, codigo, categoria, tipo, fecha_ingreso, estado, id_usuario) VALUES (:id_activo_categoria, :codigo, :categoria, :tipo, :fecha_ingreso, :estado, :id_usuario)");
